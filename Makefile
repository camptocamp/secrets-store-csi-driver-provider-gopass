REGISTRY_NAME?=docker.io/camptocamp
IMAGE_NAME=secrets-store-csi-driver-provider-gopass
...
build: setup
	CGO_ENABLED=0 go build -a -ldflags $(LDFLAGS) -o _output/secrets-store-csi-driver-provider-gopass_$(GOOS)_$(GOARCH)_$(IMAGE_VERSION) .
